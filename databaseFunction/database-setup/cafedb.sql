...
CREATE TABLE product (product_id INT AUTO_INCREMENT PRIMARY KEY, product_name VARCHAR(50) UNIQUE NOT NULL, product_img VARCHAR(200), price FLOAT NOT NULL, available VARCHAR(20) NOT NULL, category_id INT NOT NULL, FOREIGN KEY (category_id) REFERENCES category(category_id) ON DELETE CASCADE) ENGINE=InnoDB;
